...
describe("Todo Manipulation", () => {
    ...
    test("POST Request and create Todo", async () => { 
        ...
        if (!existingUser) {
            let res = await request(app).post("/todo").send("");
        ...
        let res = await request(app).post("/todo").send(newTodo);
    ...
    test("POST Request and with invalid Todo object", async () => { 
        ...
        let res = await request(app).post("/todo").send(invalidTodo);
        expect(res.status).toEqual(500);
        });

    test("PATCH Request on created Todo", async () => {
        if (!newTodoID) {
            let res = await request(app).patch("/todo/" + newTodoID).set("done", "true").send();
    ...
    test("PATCH Bad Request on created Todo", async () => { 
        ...
        let res = await request(app).patch("/todo/" + newTodoID).set("invalid", "true").send();
        expect(res.status).toEqual(200);
        });
    ...
    test("DELETE Request on created Todo", async () => { 
        ...
        if (!createdTodo) {
            let res = await request(app).delete("/todo/" + newTodoID);
            expect(res.status).toEqual(404);
            return;
        }
    ...
    test("DELETE Request on invalid Todo ID", async () => { 
        ...
        let res = await request(app).delete("/todo/" + invalidTodoID);
        expect(res.status).toEqual(404);
        });
    });
