...
describe("Get Todos for a Job", () => {
    ...
    test("GET Request on nonexisting JobID", async () => {
        let nonExistingJobID = "72377d83f1c05be17ceea25d"; // valid 12 bit string
...
describe("Job Manipulation", () => {
    ...
    test("POST Request and create job", async () => { 
        ...
        let res = await request(app).post("/job/").send(jobDetails);
        expect(res.status).toEqual(200);
        });
    ...
    test("PATCH Request on created job", async () => {
        if (!foundID) {
            let res = await request(app).patch("/job/" + foundID).set("url", "TESTING PATCH REQ JOB").send();
            expect(res.status).toEqual(500);
            return;
        }
        let res = await request(app).patch("/job/" + foundID).set("url", "TESTING PATCH REQ JOB").send();
    ...
    test("DELETE Request on created job", async () => { 
        ...
        let res = await request(app).delete("/job/" + createdJobID);
        expect(res.status).toEqual(200);
        });
